...
# Alumina Sample
df_a = pd.read_csv (r'C:\Users\andre\Documents\GitHub\MSE-353-Lab-01\Standard - 30kV40mA.csv', \
                    usecols=[0,1], skiprows=a)
df_b = pd.read_csv (r'C:\Users\andre\Documents\GitHub\MSE-353-Lab-01\Standard - 45kV20mA.csv', \
                    usecols=[0,1], skiprows=b)
df_c = pd.read_csv (r'C:\Users\andre\Documents\GitHub\MSE-353-Lab-01\Standard - 45kV40mA.csv', \
                    usecols=[0,1], skiprows=c)
...
# XRD Data
df_d = pd.read_csv (r'C:\Users\andre\Documents\GitHub\MSE-353-Lab-01\XRD.csv', usecols=[0,1], \
                    skiprows=d)
...
# EDS Data 
df_e = pd.read_csv (r'C:\Users\andre\Documents\GitHub\MSE-353-Lab-01\EDS.csv', skiprows=e, \
...
labels = (r'$O_{K \alpha}$', r'$Na_{K \alpha}$', r'$Al_{K \alpha}$',  r'$Si_{K \alpha}$', \
          r'$Au_{M \alpha}$', r'$Fe_{K \alpha}$', r'$Fe_{K \beta}$', r'$Au_{L \alpha}$',  \
          r'$Au_{L \beta}$')

print(1000 * x_vals)

fig_3 = plt.figure(figsize=(6, 6), dpi=300)
plt.plot(df_e['Energy'][range(0,1350)], df_e['Counts'][range(0,1350)], linewidth=0.5, color='purple')
...
for i in range(0,9):
    plt.annotate(labels[i], (x_vals[i], y_vals[i]), xytext=(0,3), textcoords='offset points', \
                 ha='center')
...
sample_ppm = 2 *pd.Series(df_f['Sample D']).array * pd.Series(df_f['Standard ppm']).array / \
            (pd.Series(df_f['Standard Before']).array + pd.Series(df_f['Standard After']).array)
weight_percent = 500 * sample_ppm * pd.Series(df_f['Gravimetric Factor']).array * 100 / \
                (pd.Series(df_f['Sample Added']).array * 10**6)
